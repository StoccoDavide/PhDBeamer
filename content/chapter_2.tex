...
\begin{frame}{Solution of \aclp{DAE}}{\acsp{DAE} are not \acsp{ODE}}
  ...
  \begin{itemize}
    \item<2-> \textbf{No}, otherwise we would not have been here\footcite{petzold1982differential}
    \item<3-> Numerical integration of \acsp{DAE} is \textbf{challenging}
    \item<4-> A \textbf{reformulation} of the \acs{DAE} system is typically required
  \end{itemize}
  \vspace{1.0em}
  \centering
  \begin{tikzpicture}[scale=0.5]
    \uncover<5->{\node at (0,0) {\textbf{Two} main approaches \dots};}
    \visible<6,8->{\draw[fg_sl_color, thick, -stealth] (-2,-1) -- (-3,-2);}
    \uncover<6,8->{\node at (-5,-3) {\hi{\large Index Reduction}};}
    \visible<7>{\draw[fg_sl_color, thick, -stealth] (+2,-1) -- (+3,-2);}
...
\subsubsection{A New Algorithm for Index Reduction}
...
\begin{frame}{Solution of \aclp{DAE}}{A New Algorithm for Index Reduction}
  ...
  \begin{small}
    \qquad \textbf{\Maple{}} for symbolic manipulation \\
    \qquad \textbf{\Matlab{}} for numerical evaluation
  \end{small}}
\end{frame}

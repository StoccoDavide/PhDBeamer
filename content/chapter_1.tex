...
\begin{frame}{Introduction and Motivation}{The Simulation of Dynamic Systems}
  \begin{columns}
    \begin{column}[c]{0.6\textwidth}
      \uncover<1->{\hi{\large Why do we need simulation?}}
      \uncover<2->{\begin{itemize}
        \item \textbf{Simulation} of dynamic systems allows to \dots \\
        \begin{itemize}
      ...
      \end{itemize}}
      \vspace{0.5em}
      \uncover<3->{\hi{\large Simulations' \textbf{complexity} is increasing!}}
      \uncover<4->{\begin{itemize}
        \item Higher \textbf{fidelity}, more \textbf{complexity}, more \\
        \begin{itemize}
    ...
    \begin{column}[c]{0.6\textwidth}
      \begin{itemize}
        ...
        \begin{small}
          ...
          \qquad other types
        \end{small}
        \item<4-> The \textbf{challenges} are \dots \hic{\normalsize Speed \quad Accuracy \quad Efficiency}
      \end{itemize}
    \end{column}
    \begin{column}[c]{0.4\textwidth}
      \centering
  ...
  \begin{columns}
    \begin{column}[c]{0.45\textwidth}
      ...
      \uncover<7->{\hic{\normalsize
        ...
        \emph{What about more general cases?}\vphantom{\acp{ODE} and \acp{DAE}}
      }}
    \end{column}
    \begin{column}[c]{0.6\textwidth}
      \centering
      ...
      \begin{tabular}{>{\centering\arraybackslash} m{3.75cm} >{\centering\arraybackslash} m{3.75cm}}
        \visible<2->{\includegraphics[width=0.4\textwidth]{figures/fade_overview.png} &
        \vspace{-2.0em}$\begin{array}{c}
          \text{Direct stiffness method} \\[0.25em]
          \begin{bmatrix}
          ...
          \end{bmatrix}
        \end{array}$} \\
        \visible<4->{\vspace{-3.0em}$\msmall{\begin{array}{c}
          \text{Tire-road interaction} \\[0.25em]
...
\subsection{Dynamic Systems Described by \aclp{ODE} and \aclp{DAE}}

\begin{frame}{Introduction and Motivation}{Dynamic Systems Described by \acsp{ODE} and \acsp{DAE}}
  \begin{itemize}
    \item<1-> Typically, \textbf{dynamic systems} are described by \dots \\
      \vspace{0.5em}
      \uncover<2->{
        \setlength{\tabcolsep}{3.5em}
        \small\centering\begin{tabular}{cc}
        \hi{\acp{ODE}}                                               & \hi{\acp{DAE}} \\
      ...
      \end{tabular}}
    \vspace{0.25em}
    \item<3-> \acp{ODE} have a \textbf{complete} theoretical foundation, \acp{DAE} do not
    \item<4-> Existing tools for \acp{DAE} are not as \textbf{mature} and \textbf{efficient} as \acp{ODE} tools
    \item<5-> From an engineering perspective, this is a \textbf{problem} (\textcolor{fg_sl_color}{\faHourglassHalf\,\faDollarSign\,\faCarCrash\,\faSadTear})
    %\begin{small}
    %\qquad high level of applicability, \emph{but not very widely used!} \\
    %\qquad effective in the solution of \acp{ODE}, \emph{but what about \acp{DAE}?}
    %\end{small}
  ...
  \uncover<6->{
    ...
    \hic{\large How can we improve the state-of-the-art in the solution of \acp{DAE}?}
  }

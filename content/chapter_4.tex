...
\begin{frame}{Index Reduction Algorithm}{Objectives and Working Principle}
  \hic{How do we reduce the index of \acs{DAE} systems?}
  \begin{enumerate}
  ...
  \end{enumerate}
  \vspace{1.0em}
  \hic{Which are the prerequisites for the algorithm?}
  \begin{itemize}
  ...
  \begin{columns}
    \begin{column}[c]{0.6\textwidth}
      ...
      \uncover<3->{\begin{bbox}[Cokernel Computation]
        The cokernel $\mK$ and its orthogonal complement $\mN$ of $\mA$ are calculated using \acs{LU} or \acs{FFLU} factorizations.
      \end{bbox}}
    \end{column}
...
\begin{frame}{Index Reduction Algorithm}{The Reduced \acs{DAE} System}
  \vspace{-1.0em}
  \hic{\faCanadianMapleLeaf~Now we can generate the code for the reduced \acs{DAE} system!}
  \begin{itemize}[<+->]
